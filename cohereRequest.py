...
def process(description, posPhrases, negPhrases, mode="cosine"):
    ...
    for j in range(len(posPhrases)):
        if (mode == "euclidean"):
            curScore = np.linalg.norm(np.array(posResponse.embeddings[j]) - np.array(descResponse.embeddings))
        else:
            curScore = sklearn.metrics.pairwise.cosine_distances(posResponse.embeddings, descResponse.embeddings)
    ...
    for j in range(len(negPhrases)):
        if(mode=="euclidean"):
            curScore = np.linalg.norm(np.array(negResponse.embeddings[j]) - np.array(descResponse.embeddings))
        else:
